...
def get_ref_image(ref_id):
    ...
    makeup_image = train_transform(makeup_image)
    non_makeup_image = train_transform(non_makeup_image)
...
def histogram_matching(dstImg, refImg, index):
    index = [x.numpy() for x in index]
    dstImg = dstImg.detach().numpy()
    refImg = refImg.detach().numpy()
