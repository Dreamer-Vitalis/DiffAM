...
def get_model_list(target_model):
    ...
    for model in model_list:
        if model == 'ir152':
            ...
            fr_model.load_state_dict(torch.load('./assets/models/ir152.pth', map_location=torch.device('cpu')))
            fr_model.to("cpu")
            fr_model.eval()
        ...
        if model == 'irse50':
            ...
            fr_model.load_state_dict(torch.load('./assets/models/irse50.pth', map_location=torch.device('cpu')))
            fr_model.to("cpu")
            fr_model.eval()
            models[model].append(fr_model)
        if model == 'facenet':
            ...
            fr_model = facenet.InceptionResnetV1(num_classes=8631, device="cpu")
            fr_model.load_state_dict(torch.load('./assets/models/facenet.pth', map_location=torch.device('cpu')))
            fr_model.to("cpu")
            fr_model.eval()
            models[model].append(fr_model)
        if model == 'mobile_face':
            ...
            fr_model.load_state_dict(torch.load('./assets/models/mobile_face.pth', map_location=torch.device('cpu')))
            fr_model.to("cpu")
            fr_model.eval()
            models[model].append(fr_model)
    return models
